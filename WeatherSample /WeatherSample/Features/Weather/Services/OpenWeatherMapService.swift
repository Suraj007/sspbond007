...
// @purpose getFirstFourForecasts method is for getting the first four forecasts models
...
//retrieveWeatherInfo method is for retrieving the weather information by appending location id in the url and call the completion handler and passed the Weather builder obj
...
//generateRequestURL method generates the reuest url with the components-latitude, longitude and app id
  fileprivate func generateRequestURL(_ location: CLLocation) -> URL? {
    guard var components = URLComponents(string:urlPath) else {
      return nil
    }

    let appId = "46a22452aa7fbfc7c7cbee789d0fadec"
